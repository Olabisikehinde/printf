	...
	int len, char padd, char extra_c)
{
	...
	if (width > len)
	{
		for (i = 1; i < width - len + 1; i++)
			buf[i] = padd;
		buf[i] = '\0';
		if (flags & F_MINUS && padd == ' ')/* Asign extra char to left of buf */
		{
			if (extra_c)
				buf[--ind] = extra_c;
...
int write_unsgnd(int is_negative, int ind,
	...
	int flags, int width, int precision, int size)
{
	/* The number stored at the buf's right and starts at the position i */
	...
	if (width > len)
	{
		...
		if (flags & F_MINUS) /* Asign extra char to left of buf [buf>padd]*/
		{
			return (write(1, &buffer[ind], length) + write(1, &buf[0], i));
		}
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > len)
	{
		...
		if (flags & F_MINUS && padd == ' ')/* Asign extra char to left of buf */
		{
			...
			if (extra_c)
				buf[--ind] = extra_c;
			return (write(1, &buf[ind], len) + write(1, &buf(], i - 3));
		}
